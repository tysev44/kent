...
type KeyPoint = { t: number; y: number; width: number; z: number; spreadFactor: number };

const btn = document.querySelector<HTMLButtonElement>('button')!;
const containerEl = document.querySelector<HTMLDivElement>('.button-container > div')!;

let tween: gsap.Tween | null = null;
...
function interpolateKeyPoints(t: number): { y: number; width: number; z: number; spreadFactor: number } {
  let p1 = keyPoints[0];
...
btn.addEventListener('mouseleave', () => {
  ...
  if (anim) {
	...
	particles.forEach((p) => {
    const currentX = gsap.getProperty(p, 'x') as number;
