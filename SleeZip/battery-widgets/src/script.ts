...
createRoot(document.getElementById("root")!).render(
	<StrictMode>
		<main>
			<IconSprites />
			<BatteryWidget title={title} subtitle={subtitle} data={data} />
			<BatteryWidget title={title} subtitle={subtitle} data={data} forceDark={true} />
		</main>
	</StrictMode>
);
function BatteryWidget({ title, subtitle, data, forceDark = false }: BatteryWidgetProps) {
	return (
		<div className={`widget${forceDark === true ? " widget--dark" : ""}`}>
			<div className="widget__header">
				<div className="widget__title">{title}</div>
				<button className="widget__button" type="button" title="AirPlay">
					<Icon icon="airplay" />
				</button>
			</div>
			<div className="widget__subtitle">{subtitle}</div>
			<div className="widget__bars">
				{data.map((item, i) => <BatteryWidgetBar {...item} key={i} />)}
			</div>
		</div>
	);
}
function BatteryWidgetBar({ name, icon, percent }: BatteryWidgetBarProps) {
    const locale = "en-US";
    ...
    const color = colors.find(color => percent <= color.percent)?.name;
    const fillStyle = {
        height: `${percent * 100}%`
    };
    ...
    return (
        <div className="widget__bar">
            <div className={`widget__bar-fill widget__bar-fill--${color}`} style={fillStyle}></div>
            <div className="widget__bar-percent">{percentFormatted}</div>
            <div className="widget__bar-icon" title={name} aria-label={name}>
                <Icon icon={icon} />
            </div>
        </div>
    );
